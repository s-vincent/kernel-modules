 ...
 */
static irqreturn_t gpio_threaded_th(int irq, void* ident)
{
    printk(KERN_INFO "%s: GPIO top-half callback!\n", (char*)ident);
 ...
 */
static irqreturn_t gpio_threaded_bh(int irq, void* ident)
{
 ...
 */
static int __init gpio_threaded_init(void)
{
    int flags = 0;
    int ret = 0;
    ...
    if(gpio_input)
    {
        ...
        ret = request_threaded_irq(gpio_to_irq(gpio_number), gpio_threaded_th,
                gpio_threaded_bh, flags, "GPIO threaded", THIS_MODULE->name);

        if(ret != 0)
        {
            printk(KERN_ERR "%s: failed to register threaded GPIO\n",
                    THIS_MODULE->name);
            gpio_free(gpio_number);
            return ret;
        }
 ...
 */
static void __exit gpio_threaded_exit(void)
{
...
module_init(gpio_threaded_init);
module_exit(gpio_threaded_exit);
