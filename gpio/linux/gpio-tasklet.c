/*
 * gpio-tasklet - GPIO tasklet kernel module.
...
/**
 * \file gpio-tasklet.c
 * \brief GPIO tasklet module for GNU/Linux.
...
static void gpio_tasklet_bh(unsigned long arg);
static DECLARE_TASKLET(gpio_tasklet_tasklet, gpio_tasklet_bh,
...
static irqreturn_t gpio_handler(int irq, void* ident)
{
    ...
    tasklet_schedule(&gpio_tasklet_tasklet);
    return IRQ_HANDLED;
}
 ...
 */
static int __init gpio_tasklet_init(void)
{
    int flags = 0;
    int ret = 0;
    ...
    if(gpio_input)
    {
        ...
        ret = request_irq(gpio_to_irq(gpio_number), gpio_handler,
 ...
 */
static void __exit gpio_tasklet_exit(void)
{
    /* be sure no tasklet is running */
    tasklet_kill(&gpio_tasklet_tasklet);
...
module_init(gpio_tasklet_init);
module_exit(gpio_tasklet_exit);
